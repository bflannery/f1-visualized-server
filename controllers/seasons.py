...
def get_season_by_year(season_year: str, db: Session = Depends(get_db)):
    db_season = season_service.get_season_by_year(db, year=season_year)
    if db_season is None:
        raise HTTPException(status_code=404, detail=f"Season year '{season_year}' not found")
    return db_season
